...
*   **Error "CUDA out of memory" o cierre inesperado durante el análisis**: Esto ocurre si tu tarjeta gráfica tiene poca memoria de vídeo (VRAM, por ejemplo 2GB o 4GB). Para solucionarlo, abre el archivo `config.py` con un editor de texto y reduce el valor de todas las variables que terminan en `_BATCH_SIZE` (ej. de `8` tíralo a `2` o `1`). El análisis tardará un poco más, pero tu gráfica no colapsará.
